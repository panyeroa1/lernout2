...
export function EburonControlBar({
  ...
  const [isRecordMenuOpen, setIsRecordMenuOpen] = React.useState(false);
  const [recordingType, setRecordingType] = React.useState<'audio' | 'video'>('audio');

  const micMenuRef = React.useRef<HTMLDivElement | null>(null);
  const screenShareMenuRef = React.useRef<HTMLDivElement | null>(null);
  const recordMenuRef = React.useRef<HTMLDivElement | null>(null);
  const mediaRecorderRef = React.useRef<MediaRecorder | null>(null);
  const recordedChunksRef = React.useRef<Blob[]>([]);
  const recordingAudioContextRef = React.useRef<AudioContext | null>(null);
  ...
  const copyInviteLink = () => {
    ...
    navigator.clipboard.writeText(url).then(() => {
      toast.success('Orbit Link Successfully Copied', {
        style: {
          background: '#22c55e',
          color: '#fff',
        },
  ...
  const toggleRecording = async (type?: 'audio' | 'video') => {
    ...
    const selectedType = type || recordingType;
    setRecordingType(selectedType);
    setIsRecordMenuOpen(false);

    // Start recording - capture audio and optionally video
    try {
      const tracks: MediaStreamTrack[] = [];

      // Create audio context and keep reference for background support
      const audioContext = new AudioContext();
      recordingAudioContextRef.current = audioContext;
      const destination = audioContext.createMediaStreamDestination();

      // Capture local microphone if enabled
      const micTrack = localParticipant?.getTrackPublication(Track.Source.Microphone)?.track;
      if (micTrack?.mediaStreamTrack) {
        const micSource = audioContext.createMediaStreamSource(new MediaStream([micTrack.mediaStreamTrack]));
        micSource.connect(destination);
      }

      // Capture all audio elements (remote participants)
      const audioElements = document.querySelectorAll('audio');
      audioElements.forEach((audio) => {
        try {
          if (audio instanceof HTMLAudioElement && audio.srcObject) {
            const source = audioContext.createMediaStreamSource(audio.srcObject as MediaStream);
            source.connect(destination);
          }
        } catch (e) {
          console.warn('Could not capture audio element:', e);
        }
      });
      ...
      mediaRecorder.onstop = () => {
        const fileType = selectedType === 'video' ? 'video/webm' : 'audio/webm';
        const blob = new Blob(recordedChunksRef.current, { type: fileType });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        const ext = selectedType === 'video' ? 'webm' : 'webm';
        a.download = `orbit-${selectedType}-${new Date().toISOString().slice(0, 19).replace(/:/g, '-')}.${ext}`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        toast.success(`${selectedType === 'video' ? 'Video' : 'Audio'} recording saved!`);
      ...
      setIsRecording(true);
      toast.success(`Recording ${selectedType} started`);

      // Save recording state to localStorage for persistence
      localStorage.setItem('orbit_recording_active', 'true');
      localStorage.setItem('orbit_recording_type', selectedType);
  ...
  // Clear recording state on stop
  React.useEffect(() => {
    if (!isRecording) {
      localStorage.removeItem('orbit_recording_active');
      localStorage.removeItem('orbit_recording_type');
    }
  }, [isRecording]);
    ...
    <>
      ...
      >
        ...
        <div className={`${styles.controlGroup} ${styles.controlGroupEnd}`}>
          <div className={styles.screenShareWrapper} ref={recordMenuRef}>
            <button
              ...
              onClick={() => {
                if (isRecording) {
                  ...
                  setIsRecordMenuOpen((prev) => !prev);
                }
              }}
              title={isRecording ? 'Stop recording' : 'Start recording'}
              style={isRecording ? { color: '#ef4444', animation: 'pulse 1s ease-in-out infinite' } : {}}
            >
              <RecordIcon />
            </button>
            {isRecordMenuOpen && !isRecording && (
              <div className={styles.screenShareMenu} role="dialog" aria-label="Recording Options">
                <button
                  className={styles.screenShareOption}
                  type="button"
      ...
      <div className={`${isMobileMenuOpen ? styles.mobileMenuVisible : ''}`}>
        ...
        <div className={styles.mobileMenuDrawer}>
          ...
          <div className={styles.mobileGrid}>
            ...
            <button
              className={`${styles.mobileGridItem} ${isRecording ? styles.mobileGridItemActive : ''}`}
              onClick={() => {
                if (isRecording) {
                  toggleRecording();
                } else {
                  toggleRecording('audio'); // Default to audio on mobile
                }
                setIsMobileMenuOpen(false);
              }}
              style={isRecording ? { color: '#ef4444' } : {}}
            >
              <div className={styles.mobileGridIcon}><RecordIcon /></div>
              <span className={styles.mobileGridLabel}>{isRecording ? 'Stop Rec' : 'Record'}</span>
            </button>
            ...
            >
              ...
              <span className={styles.mobileGridLabel}>Share</span>
            </button>
